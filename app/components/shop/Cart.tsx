...
const Cart: React.FC = () => {
	...
	const {
		cart: { cartItems },
		isCartVisible,
		isTopBannerVisible, // Added to state
	} = state;
	...
	return (
		<div>
			{isCartVisible && (
				<div
					id="cartContainer"
					className={`fixed right-0 top-[8.5rem] sm:top-[3.14rem] w-full sm:w-[50%] h-screen bg-blue-400 z-[10000] transform transition-transform duration-1000 ease-in-out`}
				>
					{/* Cart Header */}
					<div className="hidden sm:flex justify-between items-center p-4 border-b border-black bg-white">
						<button onClick={closeCartHandler} className="">
							<X className="h-[3.35rem] w-[3.35rem] lg:h-[4.6rem] lg:w-[4.6rem] text-black hover:bg-gray-200 rounded-full p-2" />
						</button>
					...
					{/* Cart Body */}
					{/* <div className="flex overflow-y-auto flex-col p-3 sm:p-5 h-full bg-white">
						{cartItems.length === 0 ? (
							...
							cartItems.map((item, index) => (
								<div
									className="flex justify-between items-center py-4 sm:py-6 border-b border-gray-300"
									key={item._key || index}
								>
											...
											<div className="flex items-center gap-2 sm:gap-3">
												<div className="flex items-center space-x-1 sm:space-x-2">
													<button
														...
														className="rounded-full p-1 hover:bg-gray-200 text-lg sm:text-xl font-bold w-8 h-8 flex items-center justify-center"
													>
														<MinusCircle size={24} />
													</button>
													<p className="text-xs sm:text-2xl font-serif">
														{item.quantity}
													</p>
													<button
														onClick={() =>
									...
									<div className="flex flex-col items-end gap-2 sm:gap-4">
										...
										<button
											...
											className="text-red-600 text-xs rounded-lg p-1 hover:bg-gray-100"
										>
											Remove
										</button>
									</div>
						...
						<div className="bg-white shadow-lg mb-16 p-10">
							{cartItems.length > 0 ? (
